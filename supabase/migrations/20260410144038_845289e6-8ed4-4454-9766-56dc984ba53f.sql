...
-- 1. Add missing columns to stripe_customers
ALTER TABLE public.stripe_customers ADD COLUMN IF NOT EXISTS stripe_subscription_id text;
ALTER TABLE public.stripe_customers ADD COLUMN IF NOT EXISTS stripe_price_id text;
ALTER TABLE public.stripe_customers ADD COLUMN IF NOT EXISTS subscription_status text;
ALTER TABLE public.stripe_customers ADD COLUMN IF NOT EXISTS current_period_end timestamptz;
ALTER TABLE public.stripe_customers ADD COLUMN IF NOT EXISTS cancel_at_period_end boolean DEFAULT false;

-- 2. Create missing RPCs (idempotent via CREATE OR REPLACE)
...
CREATE OR REPLACE FUNCTION public.debit_ai_credits(_user_id uuid, _feature_code text, _credits integer, _provider_cost_usd numeric DEFAULT NULL, _metadata jsonb DEFAULT '{}'::jsonb)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.credit_ai_wallet(_user_id uuid, _credits integer, _operation_type ai_ledger_type, _description text DEFAULT NULL, _stripe_payment_id text DEFAULT NULL, _public_price_chf numeric DEFAULT NULL, _metadata jsonb DEFAULT '{}'::jsonb)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
