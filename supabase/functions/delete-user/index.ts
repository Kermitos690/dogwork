...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const deleteFrom = async (table: string, col: string, id: string, errors: string[]) => {
      const { error } = await admin.from(table).delete().eq(col, id);
      if (error && error.code !== "42P01") errors.push(`${table}.${col}: ${error.message}`);
    };

    // Delete user-owned data before removing the authentication record.
    // Keep child/dependent rows before parent rows so cleanup does not silently fail.
    const tables = [
      // AI conversations / credits
      { table: "ai_messages", col: "user_id" },
      { table: "ai_conversations", col: "user_id" },
      { table: "ai_generated_documents", col: "user_id" },
      { table: "ai_agent_preferences", col: "user_id" },
      { table: "ai_credit_ledger", col: "user_id" },
      { table: "ai_credit_wallets", col: "user_id" },
      // Owner data
      ...
      { table: "course_admin_notes", col: "admin_user_id" },
      { table: "course_admin_notes", col: "educator_user_id" },
      // Shelter data
      { table: "shelter_activity_log", col: "shelter_user_id" },
      { table: "shelter_animal_evaluations", col: "coach_user_id" },
      { table: "shelter_animal_evaluations", col: "shelter_user_id" },
      { table: "shelter_coaches", col: "coach_user_id" },
      { table: "shelter_coaches", col: "shelter_user_id" },
      ...
      { table: "adoption_checkins", col: "shelter_user_id" },
      ...
      { table: "professional_alerts", col: "coach_user_id" },
      ...
      { table: "usage_tracking", col: "user_id" },
      // Roles & profile (last)
    ...
    for (const { table, col } of tables) {
      await deleteFrom(table, col, userId, errors);
    }

    // Delete the auth user
    const { error: authError } = await admin.auth.admin.deleteUser(userId);
    if (authError) {
      throw new Error(`Le compte d'authentification n'a pas été supprimé : ${authError.message}`);
    }
